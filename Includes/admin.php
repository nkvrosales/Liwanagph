<?php 
    ...
    function retrieve_bills_generated($id, $offset, $rowsperpage) {
        global $con;
        $query  = "SELECT user.name AS user, bill.bdate AS bdate, bill.month AS month, bill.units AS units, bill.amount AS amount, bill.id as bid ";
        $query .= ", bill.ddate AS ddate, bill.status AS status ";
        $query .= " FROM user INNER JOIN bill ON user.id = bill.uid ";
